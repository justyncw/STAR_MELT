...
line_resources_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "Line_Resources"))

# Spectral Type = Teff conversion file
try:
    line_resources_dir = os.path.join(os.path.dirname(__file__), "Line_Resources")
    spt_teff = pd.read_csv(os.path.join(line_resources_dir, "HH14_SpT_Teff.csv"))
except Exception as e:
    print(f"Error loading line resources directory: {e}")
    pass
...
lines_file = os.path.join(line_resources_dir, 'JCW_all_lines_2_20kA_100321.csv')
...
line_table.sort_values(['obs_wl_air', 'Aki'], ascending=[True, False], inplace=True)
line_table.drop_duplicates('obs_wl_air', inplace=True)
line_table.reset_index(drop=True, inplace=True)

xr_lines_file = os.path.join(line_resources_dir, 'JCW_XR_lines_2_50A.csv')
...
xrs_lines_file = os.path.join(line_resources_dir, 'JCW_Stelzer_XR_lines_2_50A.csv')
...
sol_lines_file = os.path.join(line_resources_dir, 'JCW_Si4_UV_line.csv')
...
prev_obs_file = os.path.join(line_resources_dir, 'previously_observed_lines_120321.csv')
prev_obs = pd.read_csv(prev_obs_file, delimiter=',', index_col=False)
prev_obs_cw21_file = os.path.join(line_resources_dir, 'JCW+21_lines.csv')
