...
data_dates_range_templ = USH.load_phot_templates()
...
filename_sel='PEN_ECha_240426.csv'




print(f"About to run full auto removal for line: '{line_sel}' with template RV calculation at: '{rv_cen_sel}'")
        ...
        if (w_min < line) & (w_max > line):
            ...
            for obs_templ in obs_templ_list:#[0:2]:
                ...
                if abs(spt_code_target - spt_code_template) <= spt_sel:
                    ...
                    rv_wl_min=rv_cen_sel-rv_range_sel
                    rv_wl_max=rv_cen_sel+rv_range_sel
                    radvel_t,vsini_t=get_rv_vsini(df_av,st_wave,st_flux,st_rv=0,date=obs_target,#adj_templ_res=USH.inst_res[USH.instrument],
                    ...
                    df_av_sub,params=subtract_templ(df_line,obs_target,target_inst,df_line_templ,obs_templ,templ_inst,
                                                    rv_shift=radvel_t,vsini=vsini_t,r=0,fs=USH.fig_size_l,plot_x=[],mask_pm=[None,4],
                                                 factor=1,shift=0,plot_subtracted=True,plot_divided=False,
                                                    return_params=True,auto_r=True,auto_vsini=True,chi_output=False,
                                                   output=False,savefig=True,savefits=True)
